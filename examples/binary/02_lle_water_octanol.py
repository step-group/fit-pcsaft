...
Loops over every water model in water_models.json, fits k_ij(T) against the
1-octanol + water LLE data, and saves the best result (lowest average ARD) to JSON.

CSV schema (examples/data/lle/1-octanol_water.csv):
    temperature_K  — temperature [K]
    x1_I           — mole fraction of 1-octanol in phase I  (water-rich)
    x1_II          — mole fraction of 1-octanol in phase II (octanol-rich)

PC-SAFT parameters for 1-octanol: Gross & Sadowski (2001), 2B association scheme.
See examples/data/parameters/alkanols_lle.json for alternative parameter sets
(1-hexanol, alternative octanol).
"""
...
params_path = [
    EXAMPLES_DIR / "data" / "parameters" / "alkanols_lle.json",
...
def main() -> None:
    plots_dir = EXAMPLES_DIR / "out" / "water_octanol_models"
    ...
    best_result.plot_kij(path=EXAMPLES_DIR / "out" / "water_octanol_kij.png")
    best_result.plot(
        path=EXAMPLES_DIR / "out" / "water_octanol_lle.png", plot_unfitted=True
    )
