...
Workflow
--------
1. Fit viscosity for each pure component (see examples/pure/09_*_viscosity.py).
2. Build a binary EOS combining both records.
3. Compute mixture viscosity and compare to experimental data.

Data
----
Replace the viscosity CSV paths with real measurements.
...
# --------------------------------------------------------------------------
# 1. PC-SAFT + viscosity parameters for each pure component
#
#    Option A: load from a JSON that already has "viscosity" filled in
#        params1 = feos.Parameters.from_json(["hexane"], "hexane_with_viscosity.json")
#        params2 = feos.Parameters.from_json(["heptane"], "heptane_with_viscosity.json")
#
#    Option B (shown here): build manually and fit viscosity inline
# --------------------------------------------------------------------------
...
def main() -> None:
    # -----------------------------------------------------------------------
    # 2. Fit viscosity parameters for each pure component
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # 3. Build binary EOS combining both records with viscosity parameters
    #    Note: use the *original* Parameters objects (without viscosity) to
    #    read back pure records, then inject viscosity via _add_viscosity.
    #    Accessing eos.parameters.pure_records on an EOS that already carries
    #    viscosity params triggers a feos deserialization panic.
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # 4. Predict mixture viscosity and compare to experimental data
    # -----------------------------------------------------------------------
