...
- **`_utils.py`**: Shared helpers — `_load_pure_records`, `_build_binary_eos`, `_kij_at_T`, `_apply_induced_association`, `_make_binary_jac_fn`.
...
- **`_csv.py`**: CSV loaders (`load_psat_csv`, `load_density_csv`, `load_hvap_csv`).
...
- Default CSV units: K, kPa, kg/m³, kJ/mol
...
`FitResult.to_json(path)` appends/updates a list of feos `PureRecord`-compatible dicts. These files are loaded by `BinaryKijFitter` via `params_path`.
