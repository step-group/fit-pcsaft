...
- **`pareto.py`**: `fit_pure_pareto(...)` → `ParetoResult`, solved with pymoo MOEA/D, in its ParallelMOEAD variant (derivative-free — large regions of parameter space have no VLE at all; plain MOEAD is loopwise and would feed the worker pool one point at a time).
  ...
  - **Do not validate against Table 1's parameters, even though they look close.** `.select()` returns `m=1.1346 σ=2.8109 ε/k=273.25 κ_ab=0.047221 ε_ab/k=3054.6` against the paper's `1.0000 / 2.9375 / 272.03 / 0.044480 / 3125.3` — 0.5% on ε/k, 2–6% on σ/κ_ab/ε_ab/k, 13% on m. It is not evidence: the water front spans AAD_vle 2.64–11.04 while the paper's own operating point for these parameters is 2.14%, outside it, because the bundled quasi-data is saturation-only over 280–620 K. A front that never reaches the paper's conditions has not reproduced the paper's parameters however close the numbers land. The cap's evidence is the AAD_sft extent, which is internal to this data set.
