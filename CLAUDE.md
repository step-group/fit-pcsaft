...
- **`pareto.py`**: `fit_pure_pareto(...)` → `ParetoResult`, solved with pymoo MOEA/D, in its ParallelMOEAD variant (derivative-free — large regions of parameter space have no VLE at all; plain MOEAD is loopwise and would feed the worker pool one point at a time).
  ...
  - **Known limitation: MOEA/D covers less of the front than NSGA-II did.** Measured on the 9600-eval water run — eq-32-reference scaling gave 190 points over AAD_sft 1.70–2.00 (eq 32 = 3.620); observed-span scaling gives 70 points over AAD_sft 1.37–1.50 (eq 32 = 2.876, largest normalized gap 0.685). NSGA-II reached AAD_sft 0.78 and eq 32 = 2.790. Spacing is excellent both ways (median normalized gap 0.0084 / 0.0064) — decomposition delivers evenness, not extent. The cause is structural: Tchebicheff's `max(w1·|f1−z1|, w2·|f2−z2|)` makes reachability depend on the ratio of objective *spans*, which is only known once you have the front, whereas NSGA-II's dominance is scale-free and its crowding distance is normalized per front. A real fix re-estimates ideal/nadir per generation **and** rescales the stored population's F (pymoo compares fresh offspring against stored F), i.e. a custom `ParallelMOEAD` subclass.
