...
uv run python examples/pure/13_water_cp_pareto.py   # three objectives, seconds
...
- **`pareto.py`**: `fit_pure_pareto(...)` → `ParetoResult`, solved with pymoo MOEA/D, in its ParallelMOEAD variant (derivative-free — large regions of parameter space have no VLE at all; plain MOEAD is loopwise and would feed the worker pool one point at a time). An `objectives` pair picks the mode — see the new bullet below.
  - **Three `objectives` tuples: two pairs at `n_obj=2`, and the `("psat", "rho", "cp")` triple at `n_obj=3` (its own bullet below).** `("vle", "sft")` (default) is Rehner & Gross 2020: `AAD_vle = mean(AARD(psat), AARD(rho))` in %, and `AAD_sft = mean|Δγ|` in mN/m — γ is an **absolute** deviation because it goes to zero at the critical point, where a relative one diverges. `("psat", "rho")` is Forte et al. 2018: the same two bulk AARDs, each on its own axis instead of pooled behind eq 30, with surface tension out of the objectives and (`sft_path` permitting) no DFT solve anywhere in the search itself — dropping `"sft"` from `objectives` also drops it from the graded-violation term (see the next bullet), so a parameter set with no stable interface is not penalized under this pair. `sft_path` stays optional and, if passed, is still loaded and still reported — see the `hvap_path` bullet below for the one place it does DFT-solve, once, in `.select()`. Unsupported pairs, and `("vle", "sft")` without `sft_path`, raise `ValueError` before fitting starts. Under that pair the search is **batched**: `_evaluate_population` hands the whole population to feos as one `feos.Property.*_derivatives` call per property (`parameter_names=[]`, one AD row per candidate per temperature, rayon-parallel in Rust) and reproduces `_evaluate_point`'s `(f1, f2, violation)` row for row (`test_evaluate_population_matches_evaluate_point_row_for_row`). Measured on this machine's locally-built feos wheel, whole-search average over a 600-evaluation budget (`pop_size=30, n_gen=20, n_restarts=1, refine=0, seed=1`, water): **0.108 ms/evaluation (0.06 s total)** against 1.49 ms (0.89 s) for the per-point loop it replaced, measured the same day on the same code otherwise; on thymol, 100 candidates × 88 temperatures cost 5 ms against 174 ms single-threaded. `("vle", "sft")` was last measured at 103.33 ms/evaluation (62.0 s over the same budget) in 2026-08 and has not been re-measured since — it still DFT-solves every point. That older run also returned 15 front points against 2; **do not** read that as a front-quality comparison — 600 evaluations at `refine=0`/`n_restarts=1` is far too small a budget for the sft mode, and the gap is a budget artefact, not a resolution one.
  - **`("psat", "rho", "cp")` is the third tuple and the only `n_obj=3` one.** It adds liquid isobaric heat capacity as its own axis: `AARD_cp` in %, on the **total** cp — feos's `Property.residual_isobaric_heat_capacity` at `(T, P)` plus the ideal-gas cp from feos's DIPPR-107 model, whose five coefficients (DIPPR units, J/(kmol·K)) the caller passes as `cp_ig`. `cp_ig` is mandatory whenever `cp_path` is given: PC-SAFT is a residual model and a bare feos EoS has no ideal-gas part (asking a `State` for total cp *panics*, with a `PanicException` that is not an `Exception`). The ideal-gas cp is evaluated once, at the data temperatures, in `_setup_pure_fit` (`_fit_utils.ideal_gas_cp`) and carried on `PureData.cp_ig`. CSV columns `T`, `cp`, optional `P`; a missing column or blank cell means saturation and is filled from psat per candidate. The triple runs on the batched in-process path (`_batched` is `"sft" not in objectives and q == 0`): one extra `vapor_pressure_derivatives` call where P is absent and one `residual_isobaric_heat_capacity_derivatives` call, row-for-row equal to `_evaluate_point` (`test_evaluate_population_matches_evaluate_point_row_for_row_with_cp`; explicit P to 1e-9, saturation to 1e-6 because psat then comes from two different feos entry points). **What is and is not measured.** Measured: the triple runs, the two evaluation paths agree, and every two-objective output is byte-identical to before (the `__str__`/`to_csv`/`_ref_dirs` pins pass unmodified). Not measured: **every MOEA/D knob in `pareto.py` — `n_neighbors=20`, `n_replace=2`, `de_f`/`de_cr`, the scaling ranking, restarts-beat-depth, the refine numbers — was measured at `n_obj=2` and has not been re-measured at 3.** What changes shape with three objectives: `_ref_dirs` rounds `pop_size` up to the Das-Dennis fan `(p+1)(p+2)/2` (60 → 66, printed when verbose; not pymoo's `"energy"` directions, which hit any count but are a stochastic optimizer); `_densify` pairs each point with its 3 nearest neighbours in normalized objective space (`_edges`) instead of consecutive sorted points — about 3n edges, so at `refine=1` the pass costs ~3× the 2-D one; `.plot()` draws three pairwise projections coloured by the third axis, no tangent; `front_quality` stays 2-D and raises. Two gotchas. With an explicit `P`, feos returns a *converged* gas-like root above the candidate's Tc (residual cp ≈ 0.0025 J/(mol·K), `ok=True`), so cp never flags "no liquid" — the bad candidate shows in `AARD_cp`, and psat/rho, which always fold into the violation, catch it. And the default `refs=(2, 2, 2)` makes `select()` the point with the lowest *mean* of the three AARDs — the Forte-pair caveat again; PC-SAFT liquid-cp errors are typically 5–15%, so pass refs that reflect that if the selection is meant to be a trade-off. Under the two pairs `cp_path` is reported-only, exactly like `hvap_path` (`test_cp_is_reported_but_never_optimized_under_the_bulk_pair`). Example: `examples/pure/13_water_cp_pareto.py`, on IAPWS-95 quasi-data (`examples/data/heat_capacity/water.csv`, saturated liquid, no P column).
  - **Two MOEA/D settings differ from pymoo's stock `ParallelMOEAD`. Everything else is pymoo's.**

    | setting | pymoo 0.6.2 | here |
    |---|---|---|
    | crossover | `SBX(prob=1.0, eta=20)` | **`DEX(F=0.5, CR=1.0)`** — DE/rand/1/bin |
    | replacement cap `n_r` | uncapped | **2** (Li & Zhang 2009) |

    Unchanged, and pinned by `test_moead_settings_that_still_match_pymoo_stock`: `n_neighbors=20`, `prob_neighbor_mating=0.9`, `Tchebicheff`, `PM(eta=20)`. The two go together: `n_r=2` is MOEA/D-DE's cap and was adopted while pymoo's SBX stayed — half the algorithm. `variant="de"` completes it.

    **Measured on five problems** (`benchmarks/*-transfer/`, `benchmarks/de-confirm/`), budget ladder × 3 seeds, scored on a shared reference front. DE wins on water, carvone, `carvone-rho6` and thymol; camphor ties. On **thymol** — the only one TESIS fits this way — at 28 800 evaluations the hv bands are disjoint (de 22.94–23.94 vs sbx 19.87–22.23), pooled coverage runs de→sbx **0.995** against sbx→de **0.020**, and eq 32 reaches **0.5096–0.5349 on all three DE seeds** against the shipped 96 000-evaluation SBX fit's **0.5610**. SBX's tail is the other half of the case: one seed returned nothing under `AAD_vle` 5%, another 7 points at igd+ 4.86. Wall time is a wash (607–644 s vs 703–707 s).

    Two explanations were raised for camphor's tie and **both were falsified** — "DE needs a complicated Pareto set" by carvone (non-associating, 3-parameter, DE wins) and "camphor's ρ axis is starved at 6 points" by `carvone-rho6` (carvone thinned to 6 density points; DE still wins). Camphor is unexplained; do not invent a third story for it.

    **Three caveats.** (1) Below ~10 000 evaluations DE is **not** established — at 2 700 the metrics disagree on every compound tested. (2) `de_cr` is *not* resolved by measurement (best-cell gap 0.24 < seed spread 0.38); `1.0` is Li & Zhang's value, not ours. `de_f=0.5` is measured. (3) DE routes every default run through the vendored `_SeededDEX.do`, which fixes a pymoo bug — a pymoo upgrade is load-bearing, see `test_dex_do_pinned_to_vendored_pymoo_version`.

    **This moves committed downstream parameters.** Any caller that does not pass `variant=` now runs DE with no diff at the call site — TESIS's `s04_fit_thymol.py` and `s04_fit_2phenylethanol.py` included. Pass `variant="sbx"` to reproduce a pre-2026-08 fit. Every benchmark under `benchmarks/` pins its operator explicitly for the same reason; `test_benchmark_sbx_arms_pin_the_operator_explicitly` keeps it that way.
  ...
  - `refine=4` (default) runs a post-search densification pass (`_densify`), and since 2026-09-02 a second one after `polish` — polished points dominate most raw neighbours, so polish alone can leave a few anchors (2-phenylethanol: 51 → 6), and the front is re-sorted by `F[:, 0]` afterwards because `polish_front` returns rows in input order (five of seven TESIS fronts came back unsorted before the fix). The pass itself: interpolate parameter vectors between adjacent front points, re-evaluate, keep whatever is non-dominated. Under NSGA-II the raw output was a population, not a curve — crowding distance preserves diversity but never forces even spacing, so it came back as clusters separated by voids (measured: 8 of 80 water points within 0.2% of each other on the AAD_vle axis, and a 2.8% stretch holding none). MOEA/D removes that: measured on the same 9600-eval water run, 190 points at median normalized spacing 0.0084 and largest gap 0.093. Every point interpolated across a void is itself non-dominated, so the voids are a sampling artefact. Interpolants are allocated in proportion to segment length in normalized objective space, not evenly — spreading them evenly wastes them inside the clusters. Measured on the 9600-eval water run: 80 → 209 points, median spacing 3.4× finer, 316 evaluations, 7 s.
  - `polish=True` solves each front point in unit-box coordinates `u = (θ − lo)/(hi − lo)`, and under `("psat", "rho")` on exact feos AD gradients (`_evaluate_with_grad`: the same two `*_derivatives` calls as the batched search, with `parameter_names=param_names(spec)`; 0.28 ms against 0.25 ms for the plain row). Measured on thymol's 170-point production front at `max_iter=50` (2026-09-05): finite differences in raw units 20.6 s, ~460 evaluations/point, 65 % of solves at `maxiter`, summed polished objective 356, eq-32 tangent 0.3992; unit box alone 22 s / Σ 139 / 0.3956; AD + unit box **11–12 s / Σ 103 / 0.3955**. Normalisation is the quality half, AD the speed half. What remains is SLSQP's line search (nfev ≈ 5/iteration even with `jac` — AARD has a kink at every data point), so `max_iter` is a fidelity knob: 150 iterations cost what 50 finite-difference ones did and reached Σ 59. The DFT pairs and the cp triple have no AD for their third objective and stay finite-difference (still in the unit box). Parallelising across points is not on the table: feos holds the GIL, `feos.Identifier` does not pickle, and each point warm-starts from its predecessor. The verbose `polish:` line prints its seconds.
...
- **Heat capacity CSV format** (`fit_pure_pareto` only): columns `T`, `cp` (aliases `heat_capacity`, `c_p`, `Cp`, `cp_liq`, `cp_J_mol_K`), optional `P` (default kPa via `pressure_unit`); a missing `P` column or a blank cell means saturation pressure. `cp` is the **total** liquid cp, default J/(mol·K) (`heat_capacity_unit`). Always paired with `cp_ig=[A, B, C, D, E]`, DIPPR-107 in J/(kmol·K). `FitResult.ard_cp` is the AARD of the total cp.
...
- **`_fit_utils.py`**: `_fetch_compound` (PubChem lookup → `feos.Identifier`); the parameter layout in one place — `param_names`/`params_dict` (the optimiser vector: `[m, sigma, epsilon_k]` + `mu` when fitted + `kappa_ab, epsilon_k_ab` when associating), `ad_model`/`ad_rows` (feos's AD model and its `[m, sigma, epsilon_k, mu(, kappa_ab, epsilon_k_ab, na, nb)]` rows) — read by `_build_record`, `jacobian.py`, `eval_pure`, `ParetoResult` and `_evaluate_population`; `_build_eos` (assembles `feos.PureRecord` + `feos.EquationOfState`); `ideal_gas_cp` (DIPPR-107 ideal-gas cp through feos's ideal-gas-only EoS, evaluated once at the cp data temperatures); `predict_bulk` (psat/rho/hvap/cp for one EoS, one vectorised `feos.Property` call per property, NaN where not converged — what `result._predict_per_property` and `_make_cost_fn` evaluate; cp is residual + `data.cp_ig`, at psat where `P_cp` is NaN); `_make_cost_fn` (weighted relative residuals), `_make_f_and_df_numerical`.
...
- **`_csv.py`**: CSV loaders (`load_psat_csv`, `load_density_csv`, `load_hvap_csv`, `load_sft_csv`, `load_cp_csv`). Schema constants (`SCHEMA_HENRY`, `SCHEMA_VISCOSITY`, etc.) define column expectations for each data type.
...
- Default heat capacity unit: J/(mol·K); `cp_ig` is always DIPPR units, J/(kmol·K)
...
- **cp** — IAPWS-95 again: the *total* isobaric cp of the saturated liquid on the same 23-point grid, written without a `P` column so the file exercises the "missing P = saturation" path. The matching DIPPR-107 constants for water are in the generator's docstring.
