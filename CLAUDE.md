...
- **`_fit_utils.py`**: `_fetch_compound` (PubChem lookup → `feos.Identifier`); the parameter layout in one place — `param_names`/`params_dict` (the optimiser vector: `[m, sigma, epsilon_k]` + `mu` when fitted + `kappa_ab, epsilon_k_ab` when associating), `ad_model`/`ad_rows` (feos's AD model and its `[m, sigma, epsilon_k, mu(, kappa_ab, epsilon_k_ab, na, nb)]` rows) — read by `_build_record`, `jacobian.py`, `eval_pure`, `ParetoResult` and `_evaluate_population`; `_build_eos` (assembles `feos.PureRecord` + `feos.EquationOfState`); `predict_bulk` (psat/rho/hvap for one EoS, one vectorised `feos.Property` call per property, NaN where not converged — what `result._predict_per_property` and `_make_cost_fn` evaluate); `_make_cost_fn` (weighted relative residuals), `_make_f_and_df_numerical`.
