...
- **`pareto.py`**: `fit_pure_pareto(...)` → `ParetoResult`, solved with pymoo MOEA/D, in its ParallelMOEAD variant (derivative-free — large regions of parameter space have no VLE at all; plain MOEAD is loopwise and would feed the worker pool one point at a time).
  ...
  - **That scaling is known to be too weak, and it costs front coverage.** Dividing by the eq-32 references equalizes the reference *values*, but Tchebicheff is governed by the *spans*: measured on water they are still 9.20 vs 0.43, i.e. 21.5:1, so the AAD_vle term wins the max for all but about four of eighty weight vectors. The 9600-eval water run reaches AAD_sft 1.70 where NSGA-II reached 0.78, and eq 32 at the tangent point is 3.62 against NSGA-II's refined 2.79 — evenly spaced, but over a sixth of the front. Normalizing by the observed objective ranges instead of the eq-32 references is the fix.
